...
export class VECTOR_UINT16 {
  ...
  fit() {
    let n: Uint16Array | null = new Uint16Array(this.count);
  ...
  push(val: number) {
    if (this.capacity < this.count + 1) {
      let n: Uint16Array | null = new Uint16Array(2 * this.capacity + 1);
  ...
  concat(array: VECTOR_UINT16) {
    const v8 = new VECTOR_UINT16();
    for (let i = 0; i < this.count; ++i) {
...
export const VectorUint16 = (...nums: any[]) => new VECTOR_UINT16(...nums);
