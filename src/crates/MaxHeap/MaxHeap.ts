...
class HeapMax {
  ...
  delete() {
    ...
    let max = Math.max(this.array[l], this.array[r] || -Infinity);

    while (p < L && this.array[p] < max) {
      let child = this.array[l] === max ? l : r;
      this.swap(p, child);
      p = child;
      l = p * 2 + 1;
      r = l + 1;
      max = Math.max(this.array[l], this.array[r] || -Infinity);
    }
    return deletedNode;
  }
...
export const maxHeap = (...arg: any[]) => new HeapMax(...arg);
